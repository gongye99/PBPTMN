...
from bulletproof.utils.utils import ModP, point_to_b64
from bulletproof.innerproduct.inner_product_verifier import Verifier1
...
class Proof:
    ...
    def to_dict(self):
        """Convert Proof instance to a dictionary."""
        # Convert innerProof to a dictionary if it has a to_dict method, otherwise keep it as is
        innerProof_dict = self.innerProof.to_dict() if hasattr(self.innerProof, 'to_dict') else self.innerProof
...
class AggregRangeVerifier:
    ...
    def to_dict(self):
        return {
            ...
            "hs": self.hs,
            "u": self.u,
            "proof": self.proof.to_dict() if hasattr(self.proof, "to_dict") else self.proof
        }
